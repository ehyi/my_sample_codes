...
class Mage_CatalogSearch_Model_Resource_Fulltext extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function prepareResult($object, $queryText, $query)
    {
        ...
        if (!$query->getIsProcessed()) {
            ...
            // EY20160712
            $sql = str_replace("IN BOOLEAN MODE", "IN NATURAL LANGUAGE MODE", $sql);
            ...
            // Move like matchings to the top.
            // Move like ^matchings to the top.
            // Move exact mathings to the top.
            $queryText_trimmed = mysql_real_escape_string(str_replace(";", "", trim($queryText)));
            $where_title_array = array(
                "and v.value like :querytext",
                "and v.value like :querytext",
                "and v.value like :querytext",
                "and v.value = :querytext"
            );
            $value_title_array = array(
                "%" . str_replace(" ", "%", $queryText_trimmed) . "%",
                "%" . $queryText_trimmed . "%",
                "" . $queryText_trimmed . "%",
                $queryText_trimmed
            );            
            for ($i = 0; $i < count($where_title_array); $i++) {
                $query2 = "
                    update catalogsearch_result r
                    inner join catalog_product_entity_varchar v on r.product_id = v.entity_id
                    inner join eav_attribute ea on v.attribute_id = ea.attribute_id
                    set r.relevance = (select truncate(max(relevance), 0) from (select * from catalogsearch_result) r2 where query_id = :query_id) + 1
                    where ea.attribute_code = 'name_full'
                    and r.query_id = :query_id
                    " . $where_title_array[$i] . "
                    ";
                $binds2 = array(
                    "query_id" => $query_id,
                    "querytext" => $value_title_array[$i]
                );
                $resource->query($query2, $binds2);
            }
            //
            $order_types = array(
                'Paperback',
            ...
            foreach ($order_types as $k => $v) {
                $relevance_val = (count($order_types) - $k) * 0.0001;
                $query2 = "
                    update catalogsearch_result c
                    inner join catalog_product_entity_varchar cp ON c.product_id = cp.entity_id 
                    set relevance = relevance + :relevance_val
                    where query_id = :query_id
                    and value = :value
                    ";
                $binds2 = array(
                    "relevance_val" => $relevance_val,
                    "query_id" => $query_id,
                    "value" => $v
                );                    
                $resource->query($query2, $binds2);
